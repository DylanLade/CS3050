...
void resetVisited(int max, Vert vert[]);
...
void sccDfs(int vertIndex, Vert vert[], SCC* scc){
    ...
    for(index = 0, mag = vert[vertIndex].magnitude; index < mag; ++index){
        ...
        if(adj > 0){
            if(!vert[adj-1].visited){
                sccDfs(adj-1, vert, scc);
            }
            else{
                addAdjVert(scc, adj);
            }
...
void stackDfs(int vertIndex, Vert vert[], Stack* stack, int max){
    ...
    for(index = 0, mag = vert[vertIndex].magnitude; index < mag; ++index){
        ...
        if(adj > 0 && !vert[adj-1].visited){
            stackDfs(adj-1, vert, stack, max);
        }
    }
